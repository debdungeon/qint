...
static int countcurnew(const char *dir, time_t *maxtime, int64_t *sizep, int *cntp);
static int countsubdir(const char *dir, const char *subdir, time_t *maxtime, int64_t *sizep, int *cntp);
...
static int docount(const char *dir, time_t *dirstamp, int64_t *sizep, int *cntp);
...
int wrapreaduserquota(const char* dir, int64_t *sizep, int *cntp)
{
time_t	tm;
time_t	maxtime;
DIR	*dirp;
...
static int maildirsize_read(const char *filename,	/* The filename */
	...
	int64_t *sizeptr,	/* Grand total of maildir size */
...
static int qcalc(int64_t s, unsigned n, const char *quota, int *percentage)
{
int64_t i;
int	spercentage=0;
int	npercentage=0;
	...
	while (quota && *quota)
	{
		...
		case 'C':
			...
			npercentage = i ? ((int64_t)n/x) * 100 / (i/x):100;
			break;
		}
	}
